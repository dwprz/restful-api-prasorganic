...
// npx jest tests/review/get-by-product-name.test.ts
...
describe("GET /api/reviews", () => {
  ...
  it("get reviews by product name should be successful", async () => {
    ...
    const result = await supertest(app)
      .get(`/api/reviews?productName=PRODUCT TEST&page=1`)
      .set("Cookie", cookies)
      .set("Authorization", AUTHORIZATION_SECRET!);
  ...
  it("get reviews by product name should fail if not admin", async () => {
    const login_result = await supertest(app)
      .post("/api/users/current/login")
      .send({
        email: user_email,
  ...
  it("get reviews by product name should fail if without page query", async () => {
    const login_result = await supertest(app)
      .post("/api/users/current/login")
      .send({
        email: admin_email,
    ...
    const result = await supertest(app)
      .get("/api/reviews?productName=PRODUCT TEST")
      .set("Cookie", cookies)
      .set("Authorization", AUTHORIZATION_SECRET!);
  ...
  it("get reviews by product name should fail if page query more than 100", async () => {
    const login_result = await supertest(app)
      .post("/api/users/current/login")
      .send({
        email: admin_email,
    ...
    const result = await supertest(app)
      .get("/api/reviews?productName=PRODUCT TEST&page=200")
      .set("Cookie", cookies)
      .set("Authorization", AUTHORIZATION_SECRET!);
  ...
  it("get reviews by product name should fail if without authorization", async () => {
    const login_result = await supertest(app)
      .post("/api/users/current/login")
      .send({
        email: admin_email,
